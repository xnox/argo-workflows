...
func TestMutexLock(t *testing.T) {
	...
	t.Run("WfLevelMutexOthernamespace", func(t *testing.T) {
		var nextWorkflow string
		concurrencyManager := NewLockManager(syncLimitFunc, func(key string) {
			nextWorkflow = key
		}, WorkflowExistenceFunc)
		...
		wf2 := wf.DeepCopy()
		wf3 := wf.DeepCopy()
		status, wfUpdate, msg, failedLockName, err := concurrencyManager.TryAcquire(wf, "", wf.Spec.Synchronization)
		require.NoError(t, err)
		assert.Empty(t, msg)
		assert.Empty(t, failedLockName)
		assert.True(t, status)
		...
		wf1.Name = "two"
		wf1.Namespace = "two"
		status, wfUpdate, msg, failedLockName, err = concurrencyManager.TryAcquire(wf1, "", wf1.Spec.Synchronization)
		require.NoError(t, err)
		assert.NotEmpty(t, msg)
		...
		wf2.Spec.Priority = pointer.Int32(5)
		holderKey2 := getHolderKey(wf2, "")
		status, wfUpdate, msg, failedLockName, err = concurrencyManager.TryAcquire(wf2, "", wf2.Spec.Synchronization)
		require.NoError(t, err)
		assert.NotEmpty(t, msg)
		assert.Equal(t, "other/Mutex/test", failedLockName)
		assert.False(t, status)
		...
		wf3.Name = "four"
		wf3.Namespace = "four"
		status, wfUpdate, msg, failedLockName, err = concurrencyManager.TryAcquire(wf3, "", wf3.Spec.Synchronization)
		require.NoError(t, err)
		assert.NotEmpty(t, msg)
		assert.Equal(t, "other/Mutex/test", failedLockName)
		assert.False(t, status)
		...
		concurrencyManager.Release(wf, "", wf.Spec.Synchronization)
		assert.Equal(t, holderKey2, nextWorkflow)
		require.NotNil(t, wf.Status.Synchronization)
		assert.Equal(t, 0, len(wf.Status.Synchronization.Mutex.Holding))
		...
		require.NotNil(t, wf2.Status.Synchronization.Mutex)
		assert.Equal(t, wf2.Name, wf2.Status.Synchronization.Mutex.Holding[0].Holder)
		concurrencyManager.ReleaseAll(wf2)
		assert.Nil(t, wf2.Status.Synchronization)
	})
}
...
func TestMutexTmplLevel(t *testing.T) {
	...
	t.Run("TemplateLevelAcquireAndRelease", func(t *testing.T) {
		...
		status, wfUpdate, msg, failedLockName, err := syncManager.TryAcquire(wf, "synchronization-tmpl-level-mutex-vjcdk-3941195474", tmpl.Synchronization)
		require.NoError(t, err)
		assert.Empty(t, msg)
		assert.Empty(t, failedLockName)
		assert.True(t, status)
		...
		// Try to acquire again
		status, wfUpdate, msg, failedLockName, err = syncManager.TryAcquire(wf, "synchronization-tmpl-level-mutex-vjcdk-2216915482", tmpl.Synchronization)
		require.NoError(t, err)
		assert.True(t, wfUpdate)
		...
		status, wfUpdate, msg, failedLockName, err = syncManager.TryAcquire(wf, "synchronization-tmpl-level-mutex-vjcdk-1432992664", tmpl.Synchronization)
		require.NoError(t, err)
		assert.NotEmpty(t, msg)
		assert.Equal(t, "default/Mutex/welcome", failedLockName)
		assert.False(t, wfUpdate)
		...
		assert.Equal(t, "synchronization-tmpl-level-mutex-vjcdk-3941195474", wf.Status.Synchronization.Mutex.Holding[0].Holder)
		syncManager.Release(wf, "synchronization-tmpl-level-mutex-vjcdk-3941195474", tmpl.Synchronization)
		require.NotNil(t, wf.Status.Synchronization)
		require.NotNil(t, wf.Status.Synchronization.Mutex)
